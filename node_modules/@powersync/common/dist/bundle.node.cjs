...
class AbstractQueryProcessor extends MetaBaseObserver {
    ...
    async updateSettingsInternal(settings, signal) {
        ...
        this.iterateListeners((l) => l[exports.WatchedQueryListenerEvent.SETTINGS_WILL_UPDATE]?.());
...
function requireCodecs () {
	...
	(function (exports$1) {
		...
		function serializeFrameWithLength(frame) {
		    ...
		    var lengthPrefixed = node_buffer.Buffer.allocUnsafe(buffer.length + UINT24_SIZE);
		...
		function serializeSetupFrame(frame) {
		    ...
		    var metadataMimeTypeLength = frame.metadataMimeType != null
		        ? node_buffer.Buffer.byteLength(frame.metadataMimeType, "ascii")
		        : 0;
		    var dataMimeTypeLength = frame.dataMimeType != null
		        ? node_buffer.Buffer.byteLength(frame.dataMimeType, "ascii")
		        : 0;
		    var payloadLength = getPayloadLength(frame);
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE +
		        SETUP_FIXED_SIZE + //
		...
		function serializeErrorFrame(frame) {
		    var messageLength = frame.message != null ? node_buffer.Buffer.byteLength(frame.message, "utf8") : 0;
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + ERROR_FIXED_SIZE + messageLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeKeepAliveFrame(frame) {
		    ...
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + KEEPALIVE_FIXED_SIZE + dataLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeLeaseFrame(frame) {
		    ...
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + LEASE_FIXED_SIZE + metaLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeRequestFrame(frame) {
		    var payloadLength = getPayloadLength(frame);
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + payloadLength);
		    var offset = writeHeader(frame, buffer);
		    writePayload(frame, buffer, offset);
		    return buffer;
		}
		...
		function serializeMetadataPushFrame(frame) {
		    ...
		    if (metadata != null) {
		        var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + metadata.byteLength);
		        var offset = writeHeader(frame, buffer);
		...
		function serializeRequestManyFrame(frame) {
		    var payloadLength = getPayloadLength(frame);
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + REQUEST_MANY_HEADER + payloadLength);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeRequestNFrame(frame) {
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + REQUEST_N_HEADER);
		    var offset = writeHeader(frame, buffer);
		...
		function serializeResumeFrame(frame) {
		    ...
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + RESUME_FIXED_SIZE + resumeTokenLength);
		    var offset = writeHeader(frame, buffer);
		    offset = buffer.writeUInt16BE(frame.majorVersion, offset);
		    offset = buffer.writeUInt16BE(frame.minorVersion, offset);
		    offset = buffer.writeUInt16BE(resumeTokenLength, offset);
		...
		function serializeResumeOkFrame(frame) {
		    var buffer = node_buffer.Buffer.allocUnsafe(FRAME_HEADER_SIZE + RESUME_OK_FIXED_SIZE);
		    var offset = writeHeader(frame, buffer);
		    writeUInt64BE(buffer, frame.clientPosition, offset);
		    return buffer;
		}
	    ...
	    return __generator(this, function (_c) {
	        switch (_c.label) {
	            case 0:
	                ...
	                metadata = node_buffer.Buffer.allocUnsafe(0);
	                return [3 /*break*/, 6];
	            case 1:
	                metadataPosition = 0;
...
function requireReassembler () {
	...
	function add(holder, dataFragment, metadataFragment) {
	    ...
	    holder.data = holder.data
	        ? node_buffer.Buffer.concat([holder.data, dataFragment])
	        : dataFragment;
	    if (holder.metadata && metadataFragment) {
	        holder.metadata = node_buffer.Buffer.concat([holder.metadata, metadataFragment]);
	    }
	    return true;
	}
	Reassembler.add = add;
	function reassemble(holder, dataFragment, metadataFragment) {
	    ...
	    if (holder.metadata) {
	        var metadata = metadataFragment
	            ? node_buffer.Buffer.concat([holder.metadata, metadataFragment])
	            : holder.metadata;
	...
	var WebsocketDuplexConnection$1 = /** @class */ (function (_super) {
	    ...
	    function WebsocketDuplexConnection(websocket, deserializer, multiplexerDemultiplexerFactory) {
	        ...
	        _this.handleMessage = function (message) {
	            try {
	                var buffer = node_buffer.Buffer.from(message.data);
...
})(exports.FetchStrategy || (exports.FetchStrategy = {}));
/**
...
class AbstractPowerSyncDatabase extends BaseObserver {
    ...
    async getUploadQueueStats(includeSize) {
        return this.readTransaction(async (tx) => {
            if (includeSize) {
                const result = await tx.execute(`SELECT SUM(cast(data as blob) + 20) as size, count(*) as count FROM ${exports.PSInternalTable.CRUD}`);
                const row = result.rows.item(0);
                return new UploadQueueStats(row?.count ?? 0, row?.size ?? 0);
            }
            else {
                const result = await tx.execute(`SELECT count(*) as count FROM ${exports.PSInternalTable.CRUD}`);
                const row = result.rows.item(0);
    ...
    async getCrudBatch(limit = DEFAULT_CRUD_BATCH_LIMIT) {
        const result = await this.getAll(`SELECT id, tx_id, data FROM ${exports.PSInternalTable.CRUD} ORDER BY id ASC LIMIT ?`, [limit + 1]);
...
SELECT * FROM crud_entries;
    ...
    async handleCrudCheckpoint(lastClientId, writeCheckpoint) {
        return this.writeTransaction(async (tx) => {
            await tx.execute(`DELETE FROM ${exports.PSInternalTable.CRUD} WHERE id <= ?`, [lastClientId]);
            if (writeCheckpoint) {
                const check = await tx.execute(`SELECT 1 FROM ${exports.PSInternalTable.CRUD} LIMIT 1`);
                if (!check.rows?.length) {
                    await tx.execute(`UPDATE ${exports.PSInternalTable.BUCKETS} SET target_op = CAST(? as INTEGER) WHERE name='$local'`, [
                        writeCheckpoint
                    ]);
                }
            }
    ...
    constructor(db, logger = Logger.get('SqliteBucketStorage')) {
        ...
        this.updateListener = db.registerListener({
            tablesUpdated: (update) => {
                ...
                if (tables.includes(exports.PSInternalTable.CRUD)) {
...
exports.AbstractQueryProcessor = AbstractQueryProcessor;
...
exports.DEFAULT_ROW_COMPARATOR = DEFAULT_ROW_COMPARATOR;
exports.DEFAULT_STREAMING_SYNC_OPTIONS = DEFAULT_STREAMING_SYNC_OPTIONS;
exports.DEFAULT_STREAM_CONNECTION_OPTIONS = DEFAULT_STREAM_CONNECTION_OPTIONS;
exports.DEFAULT_SYNC_CLIENT_IMPLEMENTATION = DEFAULT_SYNC_CLIENT_IMPLEMENTATION;
